...
namespace GameManager.Scenes
{
    internal class StartScene : IScene
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            _textRenderer.SetFontScale(10);
            _textRenderer.DrawString(spriteBatch, "EARTH BELOW", 
                new Vector2(Glob.ResX / 2 - _textRenderer.MeasureString("EARTH BELOW").Width / 2, 100));
            _textRenderer.SetFontScale(4);
            _textRenderer.DrawString(spriteBatch, "PRESS SPACE TO START",
                new Vector2(Glob.ResX / 2 - _textRenderer.MeasureString("PRESS SPACE TO START").Width / 2, 
                Glob.ResY - 200));
            _textRenderer.SetFontScale(2);
            _textRenderer.DrawString(spriteBatch, "PRESS F FOR FULLSCREEN",
                new Vector2(Glob.ResX / 2 - _textRenderer.MeasureString("PRESS F FOR FULLSCREEN").Width / 2, 
                Glob.ResY - 125));
        }
    }
}
