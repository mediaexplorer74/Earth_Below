...
namespace GameManager.Interface
{
    internal class Tilemap
    {
        ...
        // Settings 
        private const int _size = 16, _width = 16, _height = 9; 
        // 16:9 aspect ration fully fills the screen, any other, there will be offset

        private readonly int _offsetX = (Glob.ResX - _width * (_size * Glob.Scale)) / 2;
        private readonly int _offsetY = (Glob.ResY - _height * (_size * Glob.Scale)) / 2;
        ...
        public int CheckToChangeLevel(Rectangle playerPosition, bool tutorial)
        {
            ...
            if (playerPosition.Top >= Glob.ResY)
            {
                if (_currentLevel == 0)
                    return 0;
        ...
        private static IsolatedStorageFile fileStorage;
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < _layout.GetLength(1); i++)
            {
                for (int j = 0; j < _layout.GetLength(0); j++)
                {
                    ...
                    Rectangle destinationRectangle = new(
                        _offsetX + i * (_size * Glob.Scale),
                        _offsetY + j * (_size * Glob.Scale),
                        _size * Glob.Scale, _size * Glob.Scale
                        );
            ...
            if (_hoveringTile != null)
            {
                Rectangle hoveringTileDestination = new(
                            _offsetX + _hoveringTile.Item1 * (_size * Glob.Scale),
                            _offsetY + _hoveringTile.Item2 * (_size * Glob.Scale),
                            _size * Glob.Scale, _size * Glob.Scale
                            ); ;
        ...
        {
            int xPosition = (Coordinate.X - _offsetX) / (_size * Glob.Scale);
            int yPosition = (Coordinate.Y - _offsetY) / (_size * Glob.Scale);
        ...
        public static Point NormalizeCoordinate(Point Coordinate)
        {
            ...
            if (Coordinate.X < Glob.OffsetX)
                Coordinate.X = -1;
            else
            {
                Coordinate.X -= Glob.OffsetX;
                adjustedX = (Coordinate.X * Glob.ResX / Glob.RenderTargetWidth);
            }

            if (Coordinate.Y < Glob.OffsetY)
                Coordinate.Y = -1;
            else
            {
                Coordinate.Y -= Glob.OffsetY;
                adjustedY = (Coordinate.Y * Glob.ResY / Glob.RenderTargetHeight);
            }
        ...
        public Rectangle GetGroundCollisionBoxFromPoint(Point Location)
        {
            ...
            return new Rectangle(
                _offsetX + PositionOnMap.Item1 * (_size * Glob.Scale),
                _offsetY + PositionOnMap.Item2 * (_size * Glob.Scale) + Glob.Scale * 12,
                _size * Glob.Scale,
                _size * Glob.Scale - Glob.Scale * 12
                ); ;
        }
        ...
        {
            Rectangle colliderRectangle = new Rectangle(LeftmostPoint.X, LeftmostPoint.Y, 
                RightmostPoint.X - LeftmostPoint.X, Glob.Scale);
        ...
        private static void WriteTilemapToDatFile(int[,] tilemap, string filePath)
        {
          using (FileStream fileStream = new FileStream(filePath, FileMode.CreateNew, FileAccess.Write))
            using (BinaryWriter writer = new BinaryWriter(/*File.Open(filePath, FileMode.Create)*/fileStream))
            {
        ...
        private static int[,] ReadTilemapFromDatFile(string filePath)
        {
          using (FileStream fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            using (BinaryReader reader = new BinaryReader(/*File.Open(filePath, FileMode.Open)*/fileStream))
            {
