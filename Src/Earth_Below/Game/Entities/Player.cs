...
namespace GameManager.Entities
{
    internal class Player : Sprite
    {
        ...
        readonly int TEXTURE_OFFSET_X = 6 * Glob.Scale;
        readonly int TEXTURE_OFFSET_Y = 3 * Glob.Scale;
        readonly int HITBOX_WIDTH = Glob.Scale * 3;
        readonly int HITBOX_HEIGHT = Glob.Scale * 11;
        ...
        public Player(Texture2D Texture, Vector2 Position, int Width, int Height, Tilemap Map) 
        ...
        public Vector2 Update(GameTime gameTime)
        {
            ...
            else if ((isLeftColliderOnSolidBlock || isRightColliderOnSolidBlock) 
                && !keyboardState.IsKeyDown(Keys.S)) // S is for going down
            {
            ...
            if (!_isPlayerGrounded)
            {
                ...
                switch (changeLevel)
                {
                    case 1:
                        ...
                        Position.Y = Glob.ResY - _hitbox.Height;
                        break;
            ...
            if (_hitbox.Right + movement.X > Glob.ResX)
            {
                movement.X = Glob.ResX - _hitbox.Right;
            }
